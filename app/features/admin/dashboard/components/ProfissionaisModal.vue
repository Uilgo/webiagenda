<template>
  ...
  >
    <div class="flex flex-col gap-4 p-4">
      <!-- Dropdown para seleção de profissionais -->
      <Dropdown
      ...
      >
        ...
        <template #trigger="{ isOpen: dropdownOpen }">
          <div
            ...
            class="flex items-center w-full gap-4 p-4 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors"
          >
            ...
            <ChevronDownIcon
              class="h-4 w-4 text-gray-400 ml-2 transition-transform duration-200"
              :class="{ 'rotate-180': dropdownOpen }"
            />
          </div>
          <div
            v-else
            class="flex items-center justify-center p-4 border-2 border-dashed border-gray-300 rounded-lg text-gray-500"
          >
          ...
          <div class="w-full max-w-md">
            ...
            <div v-else class="max-h-64 overflow-y-auto">
              <DropdownItem
                ...
                :key="profissional.id_do_profissional || profissional.id_do_perfil || 'default'"
              ...
              >
                ...
                <template #trailing>
                  <p class="text-xs text-gray-500 truncate ml-2">
...
const loadCurrentProfissional = async () => {
  if (!userStore.profile || profissionais.value.length === 0) {
    await fetchProfissionais(true);
  }
  const profileId = userStore.profile?.id;
  if (profileId) {
    currentProfissional.value = profissionais.value.find(
...
const handleSelect = (profissional: ProfissionalRPC, closeDropdown: () => void) => {
